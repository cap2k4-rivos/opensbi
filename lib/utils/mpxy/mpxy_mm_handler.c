...
/** 
 * TODO: Instead of hardcoding these values
 * these should be fetched from the device tree
 * instead.
 */
#define MM_SHMEM_ADDR_LOW 0xFFE00000
#define MM_SHMEM_SIZE 0x200000
                ...
                struct sbi_domain *switch_to_domain)
{
	...
	sbi_printf("[OpenSBI MPXY] Management Mode Service Handler: channel_id: %d, message_id: %d, message_type: ", channel_info->channel.channel_id, msg_id);
	if (RISCV_MSG_ID_SMM_GET_ATTRIBUTES == msg_id) {
		sbi_printf("MM_GET_ATTRIBUTES\n");
		struct mm_get_attributes attr;
		memset(&attr, 0, sizeof(struct mm_get_attributes));
		attr.status	 = 0;
		attr.mm_version = SMM_VERSION_COMPILED;
		attr.mm_shmem_addr_low = MM_SHMEM_ADDR_LOW;
		attr.mm_shmem_size     = MM_SHMEM_SIZE;
